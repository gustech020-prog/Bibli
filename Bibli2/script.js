...
const els = {
  ...
  rentalBookSearch: document.querySelector("#rentalBookSearch"),
  rentalReaderSearch: document.querySelector("#rentalReaderSearch"),
  rentalBookList: document.querySelector("#rentalBookList"),
  rentalReaderList: document.querySelector("#rentalReaderList"),
  ...
  exportBooksBtn: document.querySelector("#exportBooksBtn"),
  exportClientsBtn: document.querySelector("#exportClientsBtn"),
  exportRentalsBtn: document.querySelector("#exportRentalsBtn"),
};
...
function exportBooksToExcel() {
  const headers = ["ID", "Título", "Autor", "Local", "Gênero", "Editora", "Status"];
  ...
  const rows = state.books.map((book) => [
    ...
    book.genre || "",
    book.publisher || "",
    rentedIds.has(book.id) ? "Emprestado" : "Disponível",
  ]);
  downloadCsv(`livros-${todayISO()}.csv`, headers, rows);
  notify("Exportação de livros concluída.");
}

function exportClientsToExcel() {
  const headers = ["ID", "Nome", "CPF", "Telefone", "Endereço"];
  const rows = state.clients.map((client) => [client.id, client.name, client.cpf || "", client.phone || "", client.address || ""]);
  downloadCsv(`leitores-${todayISO()}.csv`, headers, rows);
  notify("Exportação de leitores concluída.");
}

function exportRentalsHistoryToExcel() {
  const headers = ["Livro", "Leitor", "Início", "Vencimento", "Devolução", "Status"];

  const rows = state.rentals.map((rental) => {
    const book = getBookById(rental.bookId);
    const client = getClientById(rental.clientId);
    const overdue = !rental.returnedDate && isOverdue(rental.dueDate);
    const status = rental.returnedDate ? "Devolvido" : overdue ? "Atrasado" : "Ativo";
    return [
      book?.title || rental.bookId,
      client?.name || rental.clientId,
      fmtDate(rental.startDate),
      fmtDate(rental.dueDate),
      fmtDate(rental.returnedDate),
      status,
    ];
  });

  downloadCsv(`historico-emprestimos-${todayISO()}.csv`, headers, rows);
  notify("Exportação do histórico concluída.");
}

function renderBookOptions() {
  if (!els.rentalBookSearch || !els.rentalBookList) return;
  const selectedValue = els.rentalBookSearch.value;
  const books = availableBooks();
  const labels = books.map((book) => `${book.id} • ${book.title}`);
  els.rentalBookList.innerHTML = labels.map((label) => `<option value="${label}"></option>`).join("");
  els.rentalBookSearch.disabled = books.length === 0;
  if (!books.length) {
    els.rentalBookSearch.value = "";
    return;
  }
  const selectedStillValid = books.some((book) => selectedValue === book.id || normalizeText(selectedValue) === normalizeText(`${book.id} • ${book.title}`) || normalizeText(selectedValue) === normalizeText(book.title));
  if (!selectedStillValid) els.rentalBookSearch.value = "";
}

function renderClientOptions() {
  if (!els.rentalReaderSearch || !els.rentalReaderList) return;
  const selectedValue = els.rentalReaderSearch.value;
  const labels = state.clients.map((client) => `${client.id} • ${client.name}`);
  els.rentalReaderList.innerHTML = labels.map((label) => `<option value="${label}"></option>`).join("");
  els.rentalReaderSearch.disabled = state.clients.length === 0;
  if (!state.clients.length) {
    els.rentalReaderSearch.value = "";
    return;
  }
  const selectedStillValid = state.clients.some((client) => selectedValue === client.id || normalizeText(selectedValue) === normalizeText(`${client.id} • ${client.name}`) || normalizeText(selectedValue) === normalizeText(client.name));
  if (!selectedStillValid) els.rentalReaderSearch.value = "";
}
...
function resolveBookId(query) {
  ...
  const match = availableBooks().find((book) => value === book.id || normalized === normalizeText(book.title) || normalized === normalizeText(`${book.id} • ${book.title}`));
  return match?.id || "";
}

function resolveReaderId(query) {
  const value = String(query || "").trim();
  if (!value) return "";
  const normalized = normalizeText(value);
  const match = state.clients.find((client) => value === client.id || normalized === normalizeText(client.name) || normalized === normalizeText(`${client.id} • ${client.name}`));
  return match?.id || "";
}
  ...
  els.booksTable.innerHTML = books
    .map((book) => `<tr>
        ...
        <td class="row-actions">
          <button type="button" class="action-btn btn-secondary icon-btn" title="Editar livro" aria-label="Editar livro" data-edit-book="${book.id}">${EDIT_ICON}</button>
          <button type="button" class="action-btn btn-danger icon-btn" title="Excluir livro" aria-label="Excluir livro" data-remove-book="${book.id}">${TRASH_ICON}</button>
        </td>
      </tr>`)
    .join("");
}
...
function renderClients() {
  ...
  if (!clients.length) {
    const emptyMsg = state.clients.length ? "Nenhum leitor encontrado." : "Nenhum leitor cadastrado.";
  ...
  els.clientsTable.innerHTML = clients
    .map((client) => `<tr>
      ...
      <td class="row-actions">
        <button type="button" class="action-btn btn-secondary icon-btn" title="Editar leitor" aria-label="Editar leitor" data-edit-client="${client.id}">${EDIT_ICON}</button>
        <button type="button" class="action-btn btn-danger icon-btn" title="Excluir leitor" aria-label="Excluir leitor" data-remove-client="${client.id}">${TRASH_ICON}</button>
      </td>
    </tr>`)
    .join("");
}
...
function renderActiveRentals() {
  ...
  els.activeRentalsTable.innerHTML = rentals
    .map((rental) => {
      ...
      <td><button type="button" class="action-btn btn-primary" data-return-book="${rental.bookId}">Dar baixa</button></td>
    </tr>`;
    })
    .join("");
}
...
async function handleClientSubmit(e) {
  ...
  if (!/^[A-Za-zÀ-ÖØ-öø-ÿ\s]+$/.test(name)) return notify("Nome do leitor deve conter apenas letras.");
  ...
  try {
    ...
    await refreshAndRender();
    e.target.reset();
    notify("Leitor cadastrado com sucesso.");
  } catch (err) {
    notify(err.message);
  }
}
...
async function handleClientEditSubmit(e) {
  ...
  try {
    ...
    await refreshAndRender();
    notify("Leitor atualizado com sucesso.");
  } catch (err) {
    notify(err.message);
  }
}
...
async function handleRentalSubmit(e) {
  e.preventDefault();
  const bookId = resolveBookId(els.rentalBookSearch?.value);
  const clientId = resolveReaderId(els.rentalReaderSearch?.value);
  const startDate = els.rentalStart?.value;
  if (!bookId || !clientId || !startDate) return notify("Pesquise e selecione livro, leitor e data de início.");
...
async function deleteSelectedHistory() {
  ...
  if (!confirmAction(`Deseja realmente apagar ${selected.length} item(ns) selecionado(s) do histórico? Esta ação não pode ser desfeita.`)) return;
  try {
...
async function handleTableClick(e) {
  ...
  try {
    if (returnBook) {
      if (!confirmAction("Confirmar baixa do empréstimo? O livro será marcado como devolvido e movido para o histórico.")) return;
      await api("return_rental", { bookId: returnBook, returnedDate: todayISO() });
      notify("Livro devolvido e registrado no histórico.");
    }

    if (removeBook) {
      const book = getBookById(removeBook);
      const bookLabel = book ? `"${book.title}" (ID ${book.id})` : `ID ${removeBook}`;
      if (!confirmAction(`Deseja realmente excluir o livro ${bookLabel}? Esta ação não pode ser desfeita.`)) return;
      await api("delete_book", { id: removeBook });
      notify("Livro excluído.");
    }

    if (removeClient) {
      const client = getClientById(removeClient);
      const clientLabel = client ? `"${client.name}" (ID ${client.id})` : `ID ${removeClient}`;
      if (!confirmAction(`Deseja realmente excluir o leitor ${clientLabel}? Esta ação não pode ser desfeita.`)) return;
      await api("delete_client", { id: removeClient });
      notify("Leitor excluído.");
    }
...
async function init() {
  ...
  bindModalInteractions(els.bookEditModal);
  bindModalInteractions(els.clientEditModal);
  ...
  if (els.exportBooksBtn) els.exportBooksBtn.addEventListener("click", exportBooksToExcel);
  if (els.exportClientsBtn) els.exportClientsBtn.addEventListener("click", exportClientsToExcel);
  if (els.exportRentalsBtn) els.exportRentalsBtn.addEventListener("click", exportRentalsHistoryToExcel);
